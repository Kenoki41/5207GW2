...
covid.with.county.map <- covid.with.county.map %>%
  mutate(Economic_typology_2015_2 = cut(Economic_typology_2015, breaks=c(0, 1, 2, 3, 4, 5),labels=c("0-1","1-2","2-3","3-4","4-5"), include.lowest=TRUE))
library(mapdeck)
...
mapdeck(token = key, style = mapdeck_style("light")) %>%
  add_polygon(
    data = covid.with.county.map, 
    layer = "polygon_layer", 
    fill_colour = "Economic_typology_2015_2", 
    palette = "diverge_hsv", 
    fill_opacity = .9, 
    legend = TRUE,
    legend_options = list(title = "Number of confirmed cases"))
