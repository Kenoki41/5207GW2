...
```{r}
library(psych)
psych::describe(vars_chosen)
```

```{r Distributions economic_type}
eco_g <- ggplot(data = vars_chosen) + 
  geom_bar(
    mapping = aes(x = economic_type, fill = factor(economic_type)), #清洗后用病例数对应观察分布
    show.legend = FALSE,
    width = 1,
    na.rm = TRUE
  ) + 
...
ggplot(data = vars_chosen) +
  geom_bar(mapping = aes(x = unemployment, fill = unemployment), na.rm = TRUE)
```

```{r Distributions pop_density}
(pde.cate <- vars_chosen %>%
  mutate(pde.cate.1=dplyr::case_when(pop_density < 45 ~ '45 and less',
                   pop_density > 45 & pop_density < 120 ~
                   '45 - 120',
                   pop_density > 120 & pop_density < 260 ~
                   '120 - 260',
                   pop_density >= 260 ~ '260 and over'),
         pde.cate.1=factor(pde.cate.1, levels = c('45 and less',
                                        '45 - 120',
                                        '120 - 260',
                                        '260 and over'))) %>%
  group_by(pde.cate.1) %>%
